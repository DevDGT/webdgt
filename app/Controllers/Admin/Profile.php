...
class Profile extends BaseController
{
    ...
    public function update()
    {
        try {
            ...
            if (!Update($this->table, $validate['data'], ['id' => session('userId')]) && !$updatePhoto) {
    ...
    public function socialsStore()
    {
        try {
            ...
            if ($socials) {
                $validate = ValidateAdd($validate, 'social', 'Media sosial sudah ada');
            }

            if (!$validate['success']) {
                throw new \Exception('gagal memproses data');
            }
            if (!Create('social', $validate['data'])) {
    ...
    public function socialsDelete()
    {
        try {
            ...
            if (Delete('social', [EncKey('id') => $id]) == false) {
    ...
    public function socialsUpdate()
    {
        try {
            ...
            if ($social && Enc($social->id) != Input_('id')) {
                $validate = ValidateAdd($validate, 'social', 'Media social sudah ada');
            }
            if (!$validate['success']) {
                throw new \Exception('Error Processing Request');
            }
            if (!Update('social', $validate['data'], [EncKey('id') => Input_('id')])) {
                throw new \Exception('Tidak ada perubahan');
            }
            $result = [
                'status' => 'ok',
    ...
    public function saveWeb()
    {
        try {
            ...
            if (!Update('web', $data, ['user_id' => session('userId')])) {
                throw new \Exception('Tidak ada perubahan');
            }
