CURRENT_PATH = ADMIN_PATH + "/career/";
...
$(document).ready((function () {
		...
		columnDefs: [{
			...
			render: function (data, type, row){
				return `
					<div class="row">
						<div class="col-auto">
							<img src="${BASE_URL}/uploads/careers/${row.icon}" class="border rounded" style='width:100px;height:100px;object-fit:cover'>
						</div>
						<div class="col-10">
							<p>${row.name}</p>
	...
	$.ajax({
		...
		beforeSend: function() {
			...
			addFormInput("#formBody", [{
				...
				type: "select2",
				name: 'id_category_career',
				label: "Kategori",
				api: {
					url: `${API_PATH}data/options/category-career`,
					option: {
						value: "id",
						caption: "{name}"
					}
		...
		success: function(result) {
			"ok" == result.status ? ($("#modalForm").modal('show'),$("#modalTitle").html('Edit Career'),$("#formInput").attr('action', CURRENT_PATH + "update"), fillForm(result.data), $("#iconGan").attr("src", `${BASE_URL}/uploads/careers/${result.data.icon}`)) : msgSweetError(result.message)
		},
		error: function(err) {
			errorCode(err)
		}
	...
	$.ajax({
		...
		success: function (e) {
			validate(e.validate.input),e.validate.success&&("ok"==e.status?(toastSuccess(e.message),refreshData(),1==e.modalClose&&$("#modalForm").modal("hide"),clearInput(e.validate.input), socket.emit("affectDataTable", careerId == '' ? tableId : careerId)):toastWarning(e.message));
		},
		error: function(err) {
			errorCode(err)
		}
...
$("#btnBackToCareers").on('click', function () {
	moveRoom(tableId)
	careerId = ''
	$("#cardCareer").removeClass("d-none")
	refreshData()
})