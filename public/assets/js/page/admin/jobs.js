...
$(document).ready((function () {
	...
	table = $(tableId).DataTable({
		...
		columns: dataColumnTable(['id', 'name', 'order']),
		columnDefs: [{
			targets: [0],
			orderable: !1,
			sClass: "text-center",
	...
	$.ajax({
		...
		success: function(result) {
			"ok" == result.status ? ($("#modalForm").modal('show'),$("#modalTitle").html('Edit Pekerjaan'),$("#formInput").attr('action', CURRENT_PATH + "update"), fillForm(result.data)) : msgSweetError(result.message)
		},
		error: function(err) {
			errorCode(err)
		}
	...
	$("#modalTitle").html('Tambah Pekerjaan')
