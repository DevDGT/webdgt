CURRENT_PATH = ADMIN_PATH + "/category-product/";
tableId = "#listCatProduct"
moveRoom(tableId)
...
$(document).ready((function () {
	...
	table = $(tableId).DataTable({
		...
		order: [],
		ajax: {
			url: API_PATH + "data/category-product",
			type: "POST",
			data: {
				_token: TOKEN
			},
	...
	let id = $(this).data("id");
	$.ajax({
		url: API_PATH + "row/category-product/" + id,
		type: "post",
		data: {_token: TOKEN},
		dataType: "json",
		beforeSend: function() {
		...
		success: function(result) {
			"ok" == result.status ? ($("#modalForm").modal('show'),$("#modalTitle").html('Edit Kategori Produk'),$("#formInput").attr('action', CURRENT_PATH + "update"), fillForm(result.data)) : msgSweetError(result.message)
		},
		error: function(err) {
			errorCode(err)
		}
	...
	$("#modalTitle").html('Tambah Kategori Produk')
